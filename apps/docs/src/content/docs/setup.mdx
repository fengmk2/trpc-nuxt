...
1. ### Install dependencies
   ...
   Then add `trpc-nuxt` to `build.transpile` in your `nuxt.config.ts`. This is required because the package ships compiled output that uses Nuxt's `#imports` virtual module, which must be resolved by Nuxt's build pipeline. Without it, the bundler sees an unresolvable specifier.
